...
use amethyst::ecs::{Component, Join, Read, System, VecStorage, WriteStorage};
    ...
    fn run(&mut self, (mut transforms, mut movement_comp, time): Self::SystemData) {
        for (movement, transform) in (&mut movement_comp, &mut transforms).join() {
            ...
            transform.set_translation_x(
                obj_x + (movement.velocity[0] as f32 * time.delta_seconds()).into(),
            );
            ...
            transform
                .set_translation_y(obj_y + (movement.velocity[1] * time.delta_seconds()).into());
        }
    }
}
